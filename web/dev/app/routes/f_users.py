from flask import render_template, redirect, url_for, request, make_response, session, send_file
from .. import db
...
@app.route('/download_pdf_f_user/<int:id>')
def download_pdf_f_user(id):
    user = FUser.query.get(id)
    current_date = str(datetime.now().date())
    html = render_template('f_users/f_user_pdf.html', user=user, current_date=current_date)
    pdf_buffer = io.BytesIO()
    pisa.CreatePDF(html, pdf_buffer)
    response = make_response(pdf_buffer.getvalue())
    response.headers['Content-Type'] = 'text/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=Famille {user.libelle_famille}.pdf'
    return response


@app.route('/download_pdf_list_f_user')
def download_pdf_list_f_user():
    libelle_famille = session['libelle_famille']
    coefficient = session['coefficient']
    remarques = session['remarques']
    users = FUser.query.filter(
    ...
    current_date = str(datetime.now().date())
    html = render_template('f_users/f_user_list_pdf.html', users=users, current_date=current_date)
    pdf_buffer = io.BytesIO()
    pisa.CreatePDF(html, pdf_buffer)
    response = make_response(pdf_buffer.getvalue())
    response.headers['Content-Type'] = 'text/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=famille.pdf'
    return response