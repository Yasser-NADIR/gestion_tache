...
def users():
    ...
    if request.method == 'POST' and search_form.validate_on_submit():
        session['nom'] = search_form.nom.data
        session['age'] = search_form.age.data
        session['email'] = search_form.email.data
        session['id_f_user'] = search_form.id_f_user.data
...
@app.route('/download_pdf_user/<int:id>')
def download_pdf_user(id):
    user = User.query.get(id)
    now = str(datetime.now().date())
    html = render_template('users/user_pdf.html', user=user, now=now, labelle_famille=find_libelle_fuser_by_id)
    pdf_buffer = io.BytesIO()
    pisa.CreatePDF(html, pdf_buffer)
    response = make_response(pdf_buffer.getvalue())
    response.headers['Content-Type'] = 'text/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=user {user.nom}.pdf'
    return response

@app.route('/download_pdf_list_user')
def download_pdf_list_user():
    nom = session['nom']
    age = session['age']
    email = session['email']
    id_f_user = session['id_f_user']
    users = User.query.filter(
            and_(
                User.nom.like(f"%{nom}%"),
                User.age.like(f'%{age}%'),
                User.email.like(f'%{email}%'),
                User.id_f_user.like(f'%{id_f_user}%')
            )
        ).all()
    current_date = str(datetime.now().date())
    html = render_template('users/user_list_pdf.html', users=users, current_date=current_date, labelle_famille=find_libelle_fuser_by_id)
    pdf_buffer = io.BytesIO()
    pisa.CreatePDF(html, pdf_buffer)
    response = make_response(pdf_buffer.getvalue())
    response.headers['Content-Type'] = 'text/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=users.pdf'
    return response