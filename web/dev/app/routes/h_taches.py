...
from ..forms import HTacheForm, HEntTacheSearchForm, HDetTacheSearchForm
from ..models import HEntTache, HDetTache
from ..utils import get_nom_user_by_id, get_num_tache, get_coef_tache
from .. import db
import io
...
def h_taches():
    ...
    session['libelle_journee'] = ''
    session['date_operation'] = ''
    session['remarques'] = ''
    session['users'] = ''
    if request.method == 'POST' and search_form.validate_on_submit():
        session['libelle_journee'] = search_form.libelle_journee.data
        session['date_operation'] = search_form.date_operation.data
        session['remarques'] = search_form.remarques.data
        session['users'] = search_form.users.data
...
def add_h_taches():
    ...
    return render_template('h_taches/add_h_taches.html', form=form, int=int, coef_tache=get_coef_tache(), line=HDetTacheSearchForm())
...
def edit_h_taches(id_h_ent_tache):
    ...
    if request.method == 'POST':
        ...
        for det_tache_form in form.det_taches:
    ...
    return render_template('h_taches/edit_h_taches.html', form=form, id=id_h_ent_tache, coef_tache=get_coef_tache(), int=int, line=HDetTacheSearchForm())
...
@app.route('/download_pdf_h_tache/<int:id>')
def download_pdf_h_tache(id):
    h_ent_tache = HEntTache.query.get(id)
    h_det_taches = HDetTache.query.filter_by(id_h_l_ent_tache=id).all()
    current_date = str(datetime.now().date())
    html = render_template('h_taches/h_tache_pdf.html', h_ent_tache=h_ent_tache, h_det_taches=h_det_taches, current_date=current_date)
    pdf_buffer = io.BytesIO()
    pisa.CreatePDF(html, pdf_buffer)
    response = make_response(pdf_buffer.getvalue())
    response.headers['Content-Type'] = 'text/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=h_tache_{h_ent_tache.id_h_ent_tache}.pdf'
    return response

@app.route('/download_pdf_list_ent_taches')
def download_pdf_list_ent_taches():
    libelle_journee = session['libelle_journee']
    date_operation = session['date_operation']
    remarques = session['remarques']
    users = session['users']
    h_ent_taches = HEntTache.query.filter(
            and_(
                HEntTache.libelle_journee.like(f'%{libelle_journee}%'),
                HEntTache.date_operation.like(f'%{date_operation}%'),
                HEntTache.remarques.like(f'%{remarques}%'),
                HEntTache.id_users.like(f'%{users}%')
            )
        ).all()
    current_date = str(datetime.now().date())
    html = render_template('f_users/f_user_list_pdf.html', h_ent_taches=h_ent_taches, current_date=current_date)
    pdf_buffer = io.BytesIO()
    pisa.CreatePDF(html, pdf_buffer)
    response = make_response(pdf_buffer.getvalue())
    response.headers['Content-Type'] = 'text/pdf'
    response.headers['Content-Disposition'] = f'attachment; filename=h_ent_tache.pdf'
    return response